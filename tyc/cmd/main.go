...
import (
	...
	tycapi "github.com/realwrtoff/rest_grpc/tyc/api/gen/go/api"
...
func main() {
	...
	Must(err)
	Must(binding.Bind(&options, flag.Instance(), binding.NewEnvGetter(binding.WithEnvPrefix("TYC")), cfg))
	...
	svc, err := service.NewTycService(
		mysqlCli, redisCli, infoLog,
		service.WithTokenExpiration(options.Tyc.TokenExpiration),
		service.WithTokenMaxRequest(options.Tyc.TokenMaxRequest),
	)
	Must(err)
	...
	tycapi.RegisterTycServiceServer(rpcServer, svc)
	...
	defer cancel()
	Must(tycapi.RegisterTycServiceHandlerFromEndpoint(
